...
function parseInputs() {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(databaseArtifact)) {
    throw new Error(`Invalid database-artifact: must match pattern ^[a-zA-Z0-9_-]+$. Got: ${databaseArtifact}`);
  }
...
async function findLatestSuccessfulRun(octokit, repo, branch) {
  try {
    ...
    core.warning(`Error finding workflow runs: ${error2}`);
    return null;
  }
}
async function findDatabaseArtifact(octokit, repo, runId, artifactName) {
  try {
    const [owner, repoName] = repo.split("/");
    if (!owner || !repoName) {
      throw new Error(`Invalid repository format: ${repo}`);
    ...
    core.warning(`Error finding artifacts: ${error2}`);
    return null;
  }
}
async function downloadArtifact(repo, artifactId, downloadPath, token) {
  try {
    ...
    core.warning(`Error downloading artifact: ${error2}`);
    return false;
  }
}
async function logDatabaseStats(databasePath) {
  try {
    ...
    core.warning(`Failed to analyze database contents: ${error2}`);
  }
}
...
async function run() {
  try {
    ...
    const artifact = await findDatabaseArtifact(octokit, repo, latestRun.id, inputs.databaseArtifact);
    if (!artifact) {
...
run().catch((error2) => {
  core.setFailed(`Unhandled error: ${error2}`);
  process.exit(1);
});
export {
  run
